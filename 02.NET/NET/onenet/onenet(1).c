...
unsigned char OneNet_FillBuf(char *buf)
{
	...
	strcat(buf, text);
	memset(text, 0, sizeof(text));
	sprintf(text, "humi,%.2f;", Data_init.humiditr); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
...
unsigned char MQTT_FillBuf(char *buf)
{
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"somg_v\":\"%d\",", threshold_value_init.somg_value); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text, "\"humi_v\":\"%d\",", threshold_value_init.humi_value); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"somg\":\"%d\",", Data_init.somg); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text, "\"temp\":\"%.1f\",", Data_init.temperatuer); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text, "\"humi\":\"%.1f\",", Data_init.humiditr); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
	...
	memset(text, 0, sizeof(text));
	sprintf(text, "\"fire\":\"%d\",", Data_init.fire); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);

	memset(text, 0, sizeof(text));
	sprintf(text, "\"beep\":\"%d\"", (int)BEEPin); // Temp是数据流的一个名称，temper是温度值
	strcat(buf, text);
